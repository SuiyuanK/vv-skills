...
This installed copy is managed through CC Switch and the `vv-skills` repository. Do not run the bundled self-update helper automatically and do not replace this directory directly from the upstream repository. Only check or update the skill when the user explicitly requests it, following the third-party update workflow in the `vv-skills` root `README.md`; preserve the reviewed local safety adjustments and update the recorded upstream baseline before committing and pushing `main`.
