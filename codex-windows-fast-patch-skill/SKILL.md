...
- For Provider History Sync and other data-only repairs, keep the lifecycle user-controlled: prepare and dry-run while Codex is open, ask the user to close Codex manually, run the write from an external PowerShell, and ask the user to reopen Codex manually after verification. Do not automate stop/relaunch for these data-only repairs. Package layouts can keep helpers under `app\resources\codex.exe`, so a narrow `app\Codex.exe` filter can miss the running App; hard-coded `shell:AppsFolder` relaunches can also fail even when the repair succeeded. Do not launch a background worker that automatically stops or restarts Codex for Provider History Sync; leave the App closed after verification and instruct the user to reopen it manually.
...
- Current Codex builds can use a lightweight versioned Computer Use cache with no usable `latest` junction and no plugin-local `node_modules`; `@oai/sky` lives under `%LOCALAPPDATA%\OpenAI\Codex\runtimes\cua_node`. Some builds are descriptor-only and omit `scripts\computer-use-client.mjs`. Treat that layout as healthy when the versioned descriptor matches the installed package and importing the independent runtime exposes `sky.list_windows`, which returns an array. Builds that still ship the client script retain the legacy client-import and helper-transport checks.
- If the failure reappears after fully quitting and reopening Codex Desktop, inspect `$env:USERPROFILE\.codex\chrome-native-hosts.json`, `$env:USERPROFILE\.codex\chrome-native-hosts-v2.json` when present, and the real targets of `$env:USERPROFILE\.codex\plugins\cache\openai-bundled\chrome\latest` and `browser\latest`. Newer Chrome plugin builds may canonicalize a versioned cache path to `chrome\latest`; that is acceptable only when the junction resolves to a versioned cache directory. Stale Chrome native-host entries, or a `chrome\latest` junction that points at `$env:USERPROFILE\.codex\.tmp\bundled-marketplaces\openai-bundled\plugins\chrome`, can let Chrome native messaging lock the mutable marketplace mirror. The symptom is `bundled_plugins_marketplace_resolve_failed` with `EBUSY` on `plugins\chrome\extension-host\windows\x64`, followed by `helper paths changed` and `missing-helper-path`; rerun `scripts\install-computer-use-local.ps1` to stop the lock holder, rebuild stable browser/chrome cache copies, repoint the Chrome native messaging manifest to the stable cache path, and repair Computer Use.
...
- When `-VerifyAllBundledPluginsAvailable` is requested, every complete descriptor in the stable `openai-bundled` marketplace appears in the union of CLI `installed` and `available` JSON entries with an existing local source; no optional plugin becomes installed or enabled as a side effect.
