...
This installed copy is managed through CC Switch and the `vv-skills` repository. Do not run an in-place self-update, pull the deployed skill directory, or replace this directory directly from the upstream repository. Only check or update the skill when the user explicitly requests it, following the third-party update workflow in the `vv-skills` root `README.md`; preserve the reviewed local safety adjustments and update the recorded upstream baseline before committing and pushing `main`.

When a patch step fails, report that the upstream may already carry a fix and offer to check it through the managed `vv-skills` workflow. Continue with the installed version if the user does not authorize an update or if the upstream cannot be reached.
