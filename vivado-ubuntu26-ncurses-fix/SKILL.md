...
| 层 | 解决什么问题 | 放哪里 | 重装后 |
|---|---|---|---|
| **① 安装器目录根级** | **安装过程不卡死**（最后一步 device list 能跑完） | 安装器 `lib/lnx64.o/` 根级 | **永久有效**——安装器目录不会被重装清空，装多少次都不卡 |
| **② 三个产品根级** | **装完正常使用**（直接运行 Vivado/Vitis） | `/opt/eda/Xilinx/2025.2.1/{Vivado,Vitis,PDM}/lib/lnx64.o/` | **每次重装后必须重跑脚本补回**——全新解压只有 `Ubuntu/24/` 有库，根级没有 |

关键机制（为什么必须两层）：

- **① 为什么有效**：安装器派生 Vivado 时，`LD_LIBRARY_PATH` **自带安装器目录的 `lib/lnx64.o`**，库放那里就能被最后一步的 Vivado 子进程找到 → 不卡死。
- **② 为什么必须**：用户自己 shell 里直接运行 `vivado` 时，`LD_LIBRARY_PATH` **不包含安装器目录**；而全新解压的产品根级没有库 → 不补回就报 `libncurses.so.5` 错误、batch 脚本静默不执行。

> **一句话总结：① 让安装器能跑完，② 让装好的 Vivado 能用。重装后必补 ②。**

直接跑本 skill 附带的脚本（幂等，可重复执行，一次把①②都做了）：
...
- **重装后必须重跑一遍脚本（补 ② 产品根级）**：全新解压不会带根级副本，只有 `Ubuntu/24/` 有库。详见上文"两层修复"表。
- ① 安装器目录的修复是持久的（安装器目录不会被重装清空），所以重装**过程**本身不卡；但②必须在重装完成后补做，否则装好的 Vivado 直接运行仍会报 ncurses 错误。
